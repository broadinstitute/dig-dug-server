...
// record a page hit by a logged in user
async function hitPage(page, session) {
    let insert = `
    INSERT INTO pages (name, login, last)
    (
        SELECT ?, id, NOW() FROM logins WHERE session=?
    )
    ON DUPLICATE KEY UPDATE
        last = values(last),
        hits = hits + 1
    ...
    return await connectionPool.execute(insert, [page, session]);
}
